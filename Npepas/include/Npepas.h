...
public:
    void Inicie(double x0, double y0, double vx0, double vy0, double R0);
    void Mover(double dt);
    void RebotarEnCaja(double W, double H, double& suma_vx_colisiones, double& suma_vy_colisiones);
    void Colisionar(Bola &otra);
    bool SolapaCon(const Bola &otra) const;
 ...
 * 
 * La malla espacial divide la caja en celdas para optimizar la detección de colisiones.
 * En lugar de verificar todas las partículas entre sí (O(N²)), solo se verifican
 * las partículas que están en celdas adyacentes.
 */
struct Celda {
...
bool InicializarSistema(Bola *bolas, int N, double R, double W, double H, std::mt19937 &gen, double velocidadMedia, double dispersionVelocidad, bool usarMalla = false);

// Funciones de simulación principal
void SimularSistema(Bola *bolas, int N, double W, double H, double dt, double tmax, const std::string &archivoSalida, const std::string &archivoEnergia, const std::string &archivoPresion, double masa, double velocidadMedia, double dispersionVelocidad, bool usarMalla, bool contarCalculos);
...
void DetectarColisionesConMalla(Bola* bolas, int N, std::vector<Celda>& malla, int celdasX, int celdasY, const std::vector<int>& offsetsVecinos, const std::vector<int>& celdasUsadas, bool contarCalculos);
