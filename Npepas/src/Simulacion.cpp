...
bool InicializarSistema(Bola *bolas, int N, double R, double W, double H, mt19937 &gen, double velocidadMedia, double dispersionVelocidad, bool usarMalla) {
    choquesEntreParticulas = 0;

    if (!VerificarAforo(R, N, W, H, usarMalla)) {
        return false;
    }

    // Inicializar todas las partículas en posición (0,0) temporalmente
    for (int i = 0; i < N; i++) {
        bolas[i].Inicie(0, 0, 0, 0, R);
    }

    if (usarMalla) {
        // Disposición en malla regular: partículas organizadas en filas y columnas
        int nCols = static_cast<int>(W / (2 * R));
        int nRows = static_cast<int>(H / (2 * R));
        int contador = 0;
        ...
        for (int i = 0; i < nRows && contador < N; i++) {
            for (int j = 0; j < nCols && contador < N; j++) {
                double x = R + 2 * R * j;
                double y = R + 2 * R * i;
                bolas[contador].Inicie(x, y, 0, 0, R);
                contador++;
            }
        }
    } else {
        // Disposición aleatoria: partículas colocadas aleatoriamente sin solapamiento
        ...
        for (int i = 0; i < N; i++) {
            ...
            while (!valido && intentos < max_intentos) {
                double x = distX(gen);
                double y = distY(gen);
                bolas[i].Inicie(x, y, 0, 0, R);
                valido = true;
                // Verificar que no solape con partículas ya colocadas
                for (int j = 0; j < i; j++) {
                    if (bolas[i].SolapaCon(bolas[j])) {
                        valido = false;
                        break;
                    }
                }
            ...
            if (!valido) {
                cerr << "Error: No se pudo colocar la partícula " << i << " sin solapamiento después de " << max_intentos << " intentos.\n";
                return false;
            }
        }
    }

    // Asignar velocidades aleatorias con distribución normal
    ...
    for (int i = 0; i < N; i++) {
        ...
        bolas[i].Inicie(bolas[i].GetX(), bolas[i].GetY(), vx, vy, R);
    }
...
void SimularSistema(Bola *bolas, int N, double W, double H, double dt, double tmax, const string &archivoSalida, const string &archivoEnergia, const string &archivoPresion, double masa, double velocidadMedia, double dispersionVelocidad, bool usarMalla, bool contarCalculos) {
    // Abrir archivos de salida para guardar datos
    ...
    // Escribir encabezados de archivos
    datos << "#" << setw(12) << "t";
    for (int i = 0; i < N; i++) {
        datos << setw(12) << "x" + to_string(i + 1) << setw(12) << "y" + to_string(i + 1) << setw(12) << "vx" + to_string(i + 1) << setw(12) << "vy" + to_string(i + 1);
    }
    datos << "\n";

    energiaFile << "# t E_cinetica_total\n";
    presionFile << "# t P_velocidad P_colisiones\n";

    int pasos = static_cast<int>(tmax / dt);
    datos << fixed << setprecision(5);
    energiaFile << fixed << setprecision(5);
    presionFile << fixed << setprecision(5);

    cout << "Simulando " << pasos << " pasos de tiempo...\n";

    double suma_vx_colisiones = 0.0, suma_vy_colisiones = 0.0;
    double presion_colisiones = 0.0;

    // Calcular tamaño óptimo de celda para la malla espacial
    // Basado en la velocidad máxima esperada y el tamaño de partículas
    double tamanoCeldaVel = (velocidadMedia + dispersionVelocidad) * dt * 3.0;
    double tamanoCeldaRadio = 2.0 * bolas[0].GetR();
    ...
    // Reiniciar contadores de cálculos si se están contando
    if (contarCalculos) {
    ...
    for (int step = 0; step < pasos; step++) {
        ...
        // Cálculo de presión por colisiones con paredes
        CalcularPresionPorColisiones(bolas, N, W, H, dt, masa, suma_vx_colisiones, suma_vy_colisiones, presion_colisiones);

        // Detección de colisiones entre partículas
        if (usarMalla) {
            // Método optimizado: usar malla espacial
            ActualizarMalla(bolas, N, malla, celdasX, celdasY, celdasUsadas, tamanoCelda, W, H, contarCalculos);
            DetectarColisionesConMalla(bolas, N, malla, celdasX, celdasY, offsetsVecinos, celdasUsadas, contarCalculos);
        } else {
            // Método directo: verificar todos los pares de partículas
            for (int i = 0; i < N; i++) {
                for (int j = i + 1; j < N; j++) {
                    if (contarCalculos) {
                        calculosDistancias++;
                    ...
                    if (bolas[i].SolapaCon(bolas[j])) {
                        bolas[i].Colisionar(bolas[j]);
                    }
                }
            }
        }

        // Cálculo de presión por teoría cinética
        double presion_velocidad = CalcularPresionPorVelocidad(bolas, N, W, H, masa);

        // Escribir datos del paso actual
        datos << setw(13) << t;
        double energiaTotal = 0.0;
        for (int i = 0; i < N; i++) {
            datos << setw(12) << bolas[i].GetX() << setw(12) << bolas[i].GetY() << setw(12) << bolas[i].GetVx() << setw(12) << bolas[i].GetVy();

            double v2 = bolas[i].GetVx() * bolas[i].GetVx() + bolas[i].GetVy() * bolas[i].GetVy();
            energiaTotal += 0.5 * masa * v2;
        }
        datos << "\n";
        energiaFile << t << " " << energiaTotal << "\n";
        presionFile << t << " " << presion_velocidad << " " << presion_colisiones << "\n";
    }
